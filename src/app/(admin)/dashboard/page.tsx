...
import { MapPin, Calendar, Users, Star, TrendingUp, Eye } from 'lucide-react'
...
async function getStats() {
  ...
  const [listings, events, reviews] = await Promise.all([
    supabase.from('listings').select('id, is_featured, is_premium, rating', { count: 'exact' }),
...
export default async function DashboardPage() {
  ...
  try {
    ...
    // DB tables may not exist yet — show placeholder UI
    stats = { totalListings: 0, totalEvents: 0, totalReviews: 0, featuredListings: 0, premiumListings: 0, avgRating: '0.0', recentListings: [] }
  }
  ...
  const cards = [
    ...
    { title: 'Avg Rating',      value: `⭐ ${stats.avgRating}`, icon: Star,     color: 'text-green-500',  bg: 'bg-green-50',  href: '/listings' },
  ]
          ...
          <CardContent className="space-y-2">
            {[
              ...
              <Link key={href} href={href}>
                <button className={`w-full text-left px-4 py-3 rounded-lg text-white text-sm font-medium ${color} transition-colors`}>
                  {label}
                </button>
              </Link>
            ))}
            ...
            <ul className="space-y-2 text-sm text-slate-600">
              {stats.totalListings === 0 && (
                <li className="flex gap-2">
                  ...
                  <span>Run the SQL schema in Supabase SQL editor, then add your first listing</span>
                </li>
              )}
              <li className="flex gap-2">
              ...
              <li className="flex gap-2">
                ...
                <span>Add your ADMIN_EMAILS to .env.local</span>
              </li>
              <li className="flex gap-2">
                <span className="text-blue-500">→</span>
                <span>Deploy to Vercel for remote access</span>
              </li>
            </ul>
          </CardContent>
        </Card>
